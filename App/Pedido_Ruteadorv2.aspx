<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Pedido_Ruteadorv2.aspx.cs"
    Inherits="App2_envio_pedidov2" MasterPageFile="~/Master/MasterPage.master" %>
...
<asp:Content ID="Titulo" ContentPlaceHolderID="titulo" runat="server">
    <div class="col-lg-9">
        <h2>Maestro pedidos Geolocalizados
        </h2>
    </div>
...
<asp:Content ID="Filtro" ContentPlaceHolderID="Filtro" runat="server">
    ...
    <div class="col-lg-2 col-lg-push-3 btn-group">
        <button type="button" onclick="mostrarTabla();" class="btn btn-success">
            <span class="glyphicon glyphicon-list" />
        </button>
        <button type="button" onclick="mostrarMapa();" class="btn btn-success">
            <span class="glyphicon glyphicon-map-marker" />
        </button>
        <button type="button" onclick="mostrarAmbos();" class="btn btn-success">
...
<asp:Content ID="Contenido" ContentPlaceHolderID="Contenedor" runat="Server">
    <div id="dv_tabla" class="col-xs-6 drawer activo">
    ...
    <div id="dv_mapa" class="col-xs-6 drawer activo">
        <div style="height: 65vh" id="map2"></div>
    </div>
...
<asp:Content ID="Scripts" ContentPlaceHolderID="scripts" runat="server">
    ...
    <script type="text/javascript">
        ...
        function mostrarMapa() {
            var dv_tabla = $('#dv_tabla');
            var dv_mapa = $('#dv_mapa');
            dv_mapa.addClass('activo');
            dv_mapa.css('width', '100%');
            dv_tabla.removeClass('activo');
            dv_tabla.css('width', '0px');
            dv_tabla.css('position', 'absolute');
            return false;
        }
        function mostrarTabla() {
            var dv_tabla = $('#dv_tabla');
            var dv_mapa = $('#dv_mapa');
            dv_tabla.addClass('activo');
            dv_tabla.css('width', '100%');
            dv_mapa.removeClass('activo');
            dv_mapa.css('width', '0px');
            dv_mapa.css('position', 'absolute');
            return false;
        }
        function mostrarAmbos() {
            ...
            dv_mapa.addClass('activo');
            dv_mapa.css('width', '');
            dv_mapa.css('position', '');
            dv_tabla.addClass('activo');
            dv_tabla.css('width', '');
            dv_tabla.css('position', '');
            return false;
        }
        ...
        $(document).ready(function (e) {
            ...
            tick_recarga = setInterval(click_recarga, 240000);
        });
        ...
        function seleccionaPedido(id) {
            var row = $(`#<%=gv_listar.ClientID%> tr[data-id="${id}"]`);
            row.css('background', 'blue');
            $(`#<%=gv_listar.ClientID%> tr[data-id="${id}"]`).css('background', 'blue');
            $(`#<%=gv_listar.ClientID%> tr[data-id!="${id}"]`).css('background', '');
            calculaScroll(row.attr('data-pos'));
        }

        var markersArray = [];

        function calculaScroll(posicion) {
            const altura = $('#<%=gv_listar.ClientID%>>tbody tr').height();
            $('#<%=gv_listar.ClientID%>').parent().scrollTop(altura * posicion);
        }
