...
public partial class App2_envio_pedidov2 : System.Web.UI.Page
{
    ...
    protected void gv_listar_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            int peru_id = Convert.ToInt32(DataBinder.Eval(e.Row.DataItem, "PERU_ID"));
            e.Row.Attributes.Add("data-id", peru_id.ToString());
            e.Row.Attributes.Add("data-pos", e.Row.RowIndex.ToString());
            ...
            btn_modificar.Enabled = !(enviado || (propuesta_ruta > 0));
            btn_eliminar.Enabled = !(enviado || (propuesta_ruta > 0));
            ((HtmlInputCheckBox)e.Row.FindControl("check")).Visible = !(enviado || (propuesta_ruta > 0));
            ...
            try
            {
                latitud = Convert.ToDecimal(DataBinder.Eval(e.Row.DataItem, "PERU_LATITUD"));
                longitud = Convert.ToDecimal(DataBinder.Eval(e.Row.DataItem, "PERU_LONGITUD"));
            }
            catch (Exception ex)
            {
            }
            ...
            if (latitud < -52 || latitud > -17)
            {
                e.Row.BackColor = System.Drawing.Color.Orange;
            }
            if (longitud < -72 || longitud > -67)
            {
                e.Row.BackColor = System.Drawing.Color.Orange;
            }
               for (int i = 1; i < e.Row.Cells.Count; i++)
            {
                //   e.Row.Attributes.Add("onclick", string.Format("javascript:centrar2({0});seleccionaPedido({0});", peru_id));
                e.Row.Cells[i].Attributes.Add("onclick", string.Format("javascript:centrar2({0});seleccionaPedido({0});", peru_id));
            }
        }
    }
